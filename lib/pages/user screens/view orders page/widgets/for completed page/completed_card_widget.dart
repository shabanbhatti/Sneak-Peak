                  ...
                  Expanded(
                    ...
                    child: Consumer(
                      builder: (context, x, child) {
                        var ratings = x.watch(reviewsStreamProvider((id: ordersModals.id ?? '',productId: cartModal.id ?? '',)),
                        );
                        return ratings.when(
                          data:
                              (data) => Opacity(
                                    ...
                                    Padding(
                                      ...
                                      child: RatingBar.builder(
                                        ...
                                        onRatingUpdate: (rating) async{
                                          ...
                                          var isReviewed= await ref.read(reviewsProvider(cartModal.id ?? '',).notifier,).addReview(rating,cartModal.id ?? '',ordersModals,);
...
final reviewsStreamProvider = StreamProvider.family<double,({String id, String productId})>((ref, record) {
  var userRepo= ref.read(userCloudDbRepositoryProvider);
  return userRepo.reviewsStream(record.id, record.productId);
});
