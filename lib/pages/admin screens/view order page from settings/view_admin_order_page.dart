...
class ViewAdminOrderPage extends ConsumerStatefulWidget {
  const ViewAdminOrderPage({super.key, required this.ordersModals});
  static const pageName = 'view_admin_orders';
  final OrdersModals ordersModals;

  @override
  ConsumerState<ViewAdminOrderPage> createState() => _ViewToShipOrderPageState();
}

class _ViewToShipOrderPageState extends ConsumerState<ViewAdminOrderPage> {
  @override
  Widget build(BuildContext context) {
    var list = widget.ordersModals.productsList ?? [];
    print('view admin order page from setting build called');
    return Scaffold(
      body: Center(
                      ...
                      ? Padding(
                        ...
                        child: CustomButton(
                          ...
                          onTap: () {
                            ...
                            var isCancelled= await ref.read(ordersProvider.notifier).cancelOrder(widget.ordersModals.id ?? '',widget.ordersModals.productsList ?? [],);
                            Navigator.pop(context);
                            if (isCancelled) {
                              SnackBarHelper.show('Cancelled successfully');
