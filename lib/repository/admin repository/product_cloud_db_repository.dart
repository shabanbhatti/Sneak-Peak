...
class ProductCloudDbRepository {
  ...
  ProductCloudDbRepository({
    required this.productsService,
    required this.productStorageService,
  });
  ...
  Future<Map<String, dynamic>> getFlChartDataByBrand(String brand) async {
    try {
      return await productsService.getFlChartDataByBrand(brand);
    } on FirebaseException catch (e) {
      throw Exception(e.code);
    }
  }
  ...
  Future<void> addProduct(
    ...
    try {
      var lists = await productStorageService.addProductFileToStorage(
        fileList,
        'product_imgs',
      );
  ...
  Future<void> updateProductImages(
    ...
    try {
      var lists = await productStorageService.updateProductImages(
        imgFileList,
        imgLinks,
        imgPaths,
      );
      await productsService.updateProductsImage(
        id,
        lists.imgLinks,
        lists.imgPaths,
      );
  ...
  Future<void> addBanners(List<File> imgFileList) async {
    try {
      var lists = await productStorageService.addProductFileToStorageForBanners(
        imgFileList,
        'banner_img',
      );
      ...
      for (var index in lists) {
        String id = DateTime.now().microsecondsSinceEpoch.toString();
              await productsService.addBanners(id, index.imgLinks, index.imgPaths);
      }
